...
<c:url var ="NewURL" value="/admin-nhanVien"/>
<html>
<head>
            ...
            <div class="row">
				...
				<form id="formSubmit" class="row pr-5 pl-5">
				    ...
				    <div class="form-group col-sm-6">
				     ...
				     <div class="form-group col-sm-6">
				        <label class="control-label no-padding-right">Tên Nhân viên</label>
				        <input type="text" class="form-control" id="tenuser" name="tenuser"  placeholder="Nhập tên nhân viên" value="${model.tenuser}"/>
				    </div> 
				    ...
				    <div class="form-group col-sm-6">
				        <label class="control-label no-padding-right">Công việc của nhân viên</label>
				        <input type="text" class="form-control" id="loaiuser" name="loaiuser" placeholder="Nhập công việc của nhân viên" value="${model.loaiuser}"/>
				    </div>
				    <div class="form-group col-sm-6">
				    ...
				    <div class="form-group">
                          <label class="col-sm-3 control-label no-padding-right">Thông tin nhân viên</label>
                          <div class="col-sm-9">                                 
                              <textarea rows="" cols="" id="thongTinUser" name="thongTinUser" style="width: 820px;height: 175px">${model.thongTinUser}</textarea>
                          </div>
                     </div> 
				    <div class="form-group col-sm-12">
				        <c:if test="${not empty model.idUser}">
				            <input type="button" class="btn btn-white btn-warning btn-bold" value="Cập nhật nhân viên" id="btnAddOrUpdateNew"/>
				        </c:if>
				        <c:if test="${empty model.idUser}">
				            <input type="button" class="btn btn-white btn-warning btn-bold" value="Thêm nhân viên" id="btnAddOrUpdateNew"/>
				        </c:if>
				    </div>
...
$(document).ready(function(){
	editor = CKEDITOR.replace('thongTinUser');
});
...
$('#btnAddOrUpdateNew').click(function (e) {
    ...
    data["thongTinUser"] = editor.getData();
    var id = $('#idUser').val();
    if (id == "") {
        addNew(data);
    } else {
