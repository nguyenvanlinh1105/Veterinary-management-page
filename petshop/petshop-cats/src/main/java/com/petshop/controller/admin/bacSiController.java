...
@WebServlet(urlPatterns= {"/admin-bacsi"})
public class bacSiController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(model.getType().equals(SystemConstant.LIST)) {
			...
			model.setListResult(userService.findAllBacSi(pageble));
			model.setTotalItem(userService.getTotalItemBacSi());
			model.setTotalPage((int) Math.ceil((double) model.getTotalItem()/model.getMaxPageItem()));
			view ="/views/admin/bacsi/list.jsp";
		}else if(model.getType().equals(SystemConstant.EDIT)) {
			if(model.getIdUser()!=null) {
				model = userService.findOneBacSi(model.getIdUser());
			}
			view="/views/admin/bacsi/edit.jsp";
		}
