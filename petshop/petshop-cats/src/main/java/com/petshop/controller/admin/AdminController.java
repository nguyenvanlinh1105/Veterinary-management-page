...
@WebServlet(urlPatterns= {"/admin-dsAdmin"})
public class AdminController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(model.getType().equals(SystemConstant.LIST)) {
			...
			model.setListResult(userService.findAllAdmin(pageble));
			model.setTotalItem(userService.getTotalItemAdmin());
			model.setTotalPage((int) Math.ceil((double) model.getTotalItem()/model.getMaxPageItem()));
			view ="/views/admin/dsAdmin/list.jsp";
		}else if(model.getType().equals(SystemConstant.EDIT)) {
			if(model.getIdUser()!=null) {
				model = userService.findOneAdmin(model.getIdUser());
			}
			view="/views/admin/dsAdmin/edit.jsp";
		}
