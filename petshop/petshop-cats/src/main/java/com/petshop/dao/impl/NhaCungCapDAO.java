...
public class NhaCungCapDAO extends AbstractDAO<NhaCungCapModel>implements INhaCungCapDAO{
	...
	public NhaCungCapModel findOneNhaCungCapById(Long newId) {
		String sql = "select * from  NhaCungCap  where maNCC ='NCC08'";
		List<NhaCungCapModel> NNCList = query(sql, new NhaCungCapMapper());
		    if (!NNCList.isEmpty()) {
		        return NNCList.get(0);
		    }
		    return null; 
	}

	@Override
	public void updateNhaCungCap(NhaCungCapModel updateModel) {
		 String sql = "{CALL UpdateNhaCungCap (?, ?, ?, ?, ?, ?, ?)}";
		    update(sql, 
		    		updateModel.getMaNCC(),
		    		updateModel.getTenNCC(), 
		    		updateModel.getDiaChiNCC(), 
		    		updateModel.getSDT(), 
		    		updateModel.getNhanVienLienHe(), 
	...
	public void deleteNhaCungCap(String id) {
		String sql = "{CALL DeleteNhaCungCap (?)}";
		update(sql, id );
