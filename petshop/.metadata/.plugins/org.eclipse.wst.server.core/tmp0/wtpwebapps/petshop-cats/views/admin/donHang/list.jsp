...
<body>
	...
	<div class="main-content ml-4 pr-4">
		<form action="<c:url value='/admin-donHang'/>" id="formSubmit" method="get">
							...
							<div class="table-btn-controls d-flex justify-content-between align-items-center pr-4">
                                   ...
                                   <div class="mb-2">
                                       <a flag="info" class="dt-button buttons-colvis btn btn-white btn-primary btn-bold " data-toggle="tooltip" title='Thêm đơn hàng' href='<c:url value="/admin-donHang?type=edit"/>'>
                                           <span>
                                               <i class="fa fa-plus-circle bigger-110 purple"></i> Thêm đơn hàng
                                           </span>
                                       </a>
									...
									<table class="table table-bordered ">
										...
										<tbody>
											<c:forEach var="item" items="${model.listResult}">
												<tr>
													<td><input type="checkbox" id="check_${item.maDonHang}" value="${item.maDonHang}"></td>
													<td>${item.maDonHang}</td>
													<td>${item.idKhachHang}</td>
													<td>${item.sdtGiaoHang}</td>
													<td>${item.diaChiGiaoHang}</td>
													<td>${item.ngayGiaoHang}</td>
													<td>${item.phThucThanhToan}</td>
													...
													<td>
														<c:url var="editURL" value="/admin-donHang">
